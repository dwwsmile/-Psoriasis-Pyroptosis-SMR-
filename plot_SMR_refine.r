...
ReadSMRData = function(plotfile)
{
    ...
    keywords=scan(plotfile, what="", nlines=1, skip=skiplines); ##2
...
SMRLocusPlot = function(data=SMRData, probeNEARBY=NULL,smr_thresh=NULL, smr_thresh_plot=NULL, heidi_thresh=NULL, plotWindow=NULL,pointsize=20,max_anno_probe=16,anno_selfdef=TRUE,smrindx="",ypos=0,smr_thresh_type = "")
{
    ...
    smrindx2 <- smrindx
    smrindx = which(data$SMR[,1] %in% smrindx)
    ...
    if(length(smrindx_bonferr)>0) { #smrprobes_red =  as.character(data$SMR[smrindx_bonferr,1]) 
                                    smrprobes_red =  as.character(data$SMR[smrindx,1])
                                    smrprobes_red1 =  as.character(data$SMR[smrindx,1])
                                    smrprobes_red2 =  as.character(data$SMR[smrindx_bonferr,1])
                                    smrprobes_red2 = smrprobes_red2[!smrprobes_red2 %in% smrprobes_red1]} ## 改了
    ...
    #min.pos = min(gwasBP)-0.02;
    #max.pos = max(gwasBP)+0.02;
    ...
    #ylab = expression(paste("-", log[10], "(", italic(P), " SMR)", sep="")); ## 改了
    ...
    eqtl.lab = expression(paste("-", log[10], "(", italic(P), " QTL)", sep=""));
    ...
    if(nprobeINFO>max_anno_probe){
      ap <- as.numeric(which(probeINFO[,1] %in% smrindx2))
      if(FALSE %in% (ap %in% c(1:max_anno_probe))){
        ap.1 <- c(1:max_anno_probe)
        ap.1 <- ap.1[!ap.1 %in% ap]
        ap.index <- as.numeric(c(ap.1,ap))
      }else{
        ap.index <- c(1:max_anno_probe)
      }
      
      probeINFO=probeINFO[ap.index,]
      nprobeINFO=dim(probeINFO)[1];
    }
    ...
    for( k in 1 : nprobeINFO)  {
         hitflag=FALSE
        if(length(which(heidiprobes_solid==probeINFO[k,1]))>0 & length(which(smrprobes_red1==probeINFO[k,1]))>0) {
             hitflag=TRUE
             colplot = "maroon"; colfont=2; pchbuf=23;
        } else if(length(which(smrprobes_red1==probeINFO[k,1]))>0) {
            colplot = "maroon"; colfont=2; pchbuf=5
        } else if (length(which(heidiprobes_solid==probeINFO[k,1]))>0 & length(which(smrprobes_red2==probeINFO[k,1]))>0) { ##从这里开始改了
            hitflag=TRUE
             colplot = "navy"; colfont=1; pchbuf=23
        } else if (length(which(smrprobes_red2==probeINFO[k,1]))>0){
            colplot = "navy"; colfont=1; pchbuf=5  
        }else{
          colplot = "grey51"; colfont=1; pchbuf=5
        }
    ...
    strbuf = paste(smr_thresh_type," = ",smr_thresh, sep="")
...
SMREffectPlot = function(data=SMRData, trait_name="",cisWindow=2000, transWindow=5000, pointsize=20,title1="",title2="",Pval_title="",Pval="")
{    
    # parameters for plot
    ...
    xmid = (xmax+xmin)/2
    mtext(paste0(title1," effect sizes"), side=1, at=xmid, line=3, cex=cex_coeff)
    # y axis
    devbuf = (ymax - ymin)/5
    if(ymax!=0 & ymin!=0) {
    ...
    ymid = (ymax + ymin)/2
    mtext(paste0(title2," effect sizes"), side=2, at=ymid, line=4.5, cex=cex_coeff)
    
    mainstr1 = trait_name
    mainstr2 = substitute(paste(probeid, " (", italic(gene), ") ",sep = ""),
    list(probeid=as.character(data$probeID),
    gene=as.character(GeneID)))
    Pval <- round(data$SMR$V8[which(data$SMR$V1 == data$probeID)],5)
    mainstr3 = paste0(Pval_title," = ",Pval)
    
    mtext(mainstr1, side=3, at=xmin, adj=0, line=2.5, cex=cex_coeff)
    mtext(mainstr2, side=3, at=xmin, adj=0, line=1, cex=cex_coeff)
    mtext(mainstr3, side=3, at=xmin, adj=0, line=0, cex=cex_coeff)
    # Plot legend
    lstr = c("top cis-QTL", "cis-QTL")
    col_led = c(col_top, col_cis); pch_led = c(pch_top, pch_cis)
    if(ntrans>0) {
        lstr=c(lstr, "trans-QTL"); col_led = c(col_led, col_trans)
